...
> 它拥有极度丝滑的阻尼歌词滚动、无缝音质切换、高级的“空气感”卡片排版，以及完全自定义的莫兰迪色系主题。只需双击 `index.html` 或部署到任何静态页面托管服务（如 Cloudflare Pages, GitHub Pages），即可随时随地享受沉浸式的音乐体验。
...
2. 直接双击打开根目录下的 `index.html`。
3. 尽情享受音乐（注意：极个别浏览器可能会对本地文件的 `fetch` 请求有跨域限制，推荐使用方法二或三）。
...
这个项目的诞生离不开开源社区的无私奉献，特此向以下项目与开发者致敬：
...
如果你喜欢这个项目，欢迎提交 **Pull Request** 或者开启 **Issue** 讨论！同时别忘了点个 ⭐ **Star** 支持一下！
